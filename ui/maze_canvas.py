...
class MazeCanvas(ctk.CTkCanvas):
    def __init__(self, parent, width, height):
        super().__init__(parent, width=width, height=height, bg="white", highlightthickness=0)
        self.load_images()

    def load_images(self):
        """Tải ảnh tường, người chơi, đích đến"""
        self.wall_img = ImageTk.PhotoImage(
            Image.open("assets/wall.png").resize((CELL_SIZE, CELL_SIZE))
        )
        self.player_img = ImageTk.PhotoImage(
            Image.open("assets/player.png").resize((CELL_SIZE, CELL_SIZE))
        )
        self.goal_img = ImageTk.PhotoImage(
            Image.open("assets/goal.png").resize((CELL_SIZE, CELL_SIZE))
        )
    ...
    def draw_maze(self, maze, goal, show_grid):
        ...
        gr, gc = goal
        self.create_image(gc * CELL_SIZE, gr * CELL_SIZE, image=self.goal_img, anchor="nw", tags="goal")

    def draw_player(self, player_pos):
        """Vẽ người chơi tại vị trí hiện tại"""
        self.delete("player")
        r, c = player_pos
        self.create_image(
            c * CELL_SIZE,
    ...
    def draw_grid_lines(self, rows, cols):
        ...
        for r in range(rows + 1):
            self.create_line(
                ...
                fill=GRID_LINE_COLOR, tags="grid_line"
            )
